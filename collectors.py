...
def network_io(ignore_names: list[str] | None = None) -> list[NetIO]:
    ...
    for name, now_one in now_c.items():
        ...
        if name not in past:
            continue
        past_one = past[name]
        sent_total = getattr(now_one, "bytes_sent", 0) or 0
        recv_total = getattr(now_one, "bytes_recv", 0) or 0
        sent = max(0.0, (sent_total - (past_one.bytes_sent or 0)) / dt)
        recv = max(0.0, (recv_total - (past_one.bytes_recv or 0)) / dt)
        ret.append(
            NetIO(
                name=name,
                sent=sent,
